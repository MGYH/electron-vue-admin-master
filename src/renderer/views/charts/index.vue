<template>
  <div>
    <pagination-table v-model="formInline">
      <template v-slot:search>
        <el-form :inline="true" v-model="formInline.form">
          ...
          <el-form-item>
            <export-excel filename="测试" methodname="getGoodsList" :form="formInline.form" :exporobject="exporobject"></export-excel>
    ...
    <mg-pie-charts :data="data"></mg-pie-charts>
  </div>
</template>
...
<script>
  ...
  // import { entryReport } from '@/api/goods'
  import PagedList from '@/utils/pageList'
  import PaginationTable from '../../components/Table/PaginationTable'
  
  export default {
    name: 'index',
    components: { MgPieCharts, ExportExcel, PaginationTable },
    mounted() {
      this.onSubmit()
    },
    data() {
      return {
        data: [],
        formInline: new PagedList('/goods/findAll', 2, true),
        exporobject: { name: '名称', num: '数量' },
